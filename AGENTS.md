...
3. 如果存在 `plan.md`，严格按照 `plan.md` 的阶段顺序推进，每个阶段完成后必须等待用户手工确认验收通过，才能进入下一阶段。不得跳过阶段或自行推进
4. 如果不存在 `plan.md`，按文档直接实现
5. 修改完成后，同步更新 `prd.md` 和 `spec.md` 使文档与代码一致
6. 编写或更新对应的测试
...
每个 phase 级别的迭代完成后，必须运行以下三项检查，确保代码质量：

- **pylint** — 检查错误和严重警告（排除 PyQt5 动态导入误报、项目规定风格的 E211/E231/E252/E265 等）
- **flake8** — 检查未使用导入、文件末尾换行等实际问题（排除项目规定风格）
- **pyflakes** — 检查未使用导入和变量、语法错误

三项检查中发现的实际问题（非风格争议）必须在提交前修复。
