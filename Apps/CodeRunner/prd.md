...
窗口顶部的菜单栏，包含 File、Edit、Run、View、Help 五个菜单，所有动作均可通过菜单触发（详见 Actions 章节）。
...
关闭最后一个标签页后，窗口进入无标签页状态（编辑区域为空），不会自动新建空白标签。首次启动程序时也是无标签页状态，用户通过 New（Ctrl+N）或 Open（Ctrl+O）创建第一个标签页。
...
- C++ 语法高亮
- 自动缩进
- 括号补全（可选）：输入 `(` `{` `[` `"` `'` 时自动插入闭符号；输入 `#include <` 时自动插入 `>` 并光标置中；输入 `/*` 时自动补 `*/` 并光标置中
- 括号匹配高亮：光标停在括号上时，配对括号用背景色高亮
- 显示行号
- 当前行高亮：光标所在行用浅色背景标记，在长代码中快速定位当前编辑位置
- Tab 键：无选区时插入 Tab 制表符（非空格），Tab 制表符默认 4 个字符宽度；有选区时对选中行增加一级缩进
- Shift+Tab：有选区时对选中行减少一级缩进
- Ctrl+/ 注释/取消注释：选中多行批量加 `//`，未选中时注释当前行，所有行已以 `//` 开头则移除
- Ctrl+D 复制当前行：光标所在行（或选中行）复制一份插入到下一行，不替换剪贴板
- Ctrl+Shift+K 删除当前行：删除光标所在行或选中行
- Alt+Up/Alt+Down 上下移动当前行：调换代码行顺序
...
MainArea 右上的输入面板，顶部有一个固定的 "INPUT" 标签文字，标签右侧有一个 ✕ 清空按钮（点击清空当前标签的输入内容，方便每次做新题目时换输入数据），下方是文本编辑区域。用户在此粘贴 OJ 题目的样例输入。点击 Test 时，文本编辑区的内容将作为程序的标准输入传递（详见 Actions 节中的 Test）。**InputPanel 的内容是 per-tab 的，切换标签时输入内容随之切换，退出时随标签页一起持久化保存，重启后恢复。**

### OutputPanel

MainArea 右下的输出面板，顶部有一个固定的 "OUTPUT" 标签文字，标签右侧有一个复制按钮（点击复制全部输出到剪贴板），下方是只读文本区域。用于显示：
...
- **右侧（光标位置与编码）**：显示当前 CodeEditor 的光标位置、文件编码和编辑模式，格式为 `Ln {当前行}/{总行数}, Col {列号} | {编码} | {模式}`（如 `Ln 4/120, Col 12 | GBK | INS`）。行号和列号均从 1 开始计数，随光标移动实时更新。编辑模式为固定三字符宽度：`INS`（Insert 插入模式）或 `OVR`（Overwrite 改写模式），通过 Insert 键切换
...
| Cut | 剪切选中文本 | Edit > Cut | 否 | Ctrl+X |
| Copy | 复制选中文本 | Edit > Copy | 否 | Ctrl+C |
| Paste | 粘贴文本 | Edit > Paste | 否 | Ctrl+V |
| Find | 查找文本 | Edit > Find | 否 | Ctrl+F |
| Replace | 替换文本 | Edit > Replace | 否 | Ctrl+H |
| Goto Line | 跳转到指定行号 | Edit > Goto Line | 否 | Ctrl+G |
| Comment/Uncomment | 注释/取消注释选中行或当前行 | Edit > Comment/Uncomment | 否 | Ctrl+/ |
| Indent | 选中行增加一级缩进 | Edit > Indent | 否 | Tab（有选区时） |
| Unindent | 选中行减少一级缩进 | Edit > Unindent | 否 | Shift+Tab（有选区时） |
| Duplicate Line | 复制当前行或选中行到下一行 | Edit > Duplicate Line | 否 | Ctrl+D |
| Delete Line | 删除当前行或选中行 | Edit > Delete Line | 否 | Ctrl+Shift+K |
| Move Line Up | 当前行或选中行上移一行 | Edit > Move Line Up | 否 | Alt+Up |
| Move Line Down | 当前行或选中行下移一行 | Edit > Move Line Down | 否 | Alt+Down |
...
| About | 显示关于信息（作者和时间） | Help > About | 否 | - |
...
选中多行时对每行行首添加 `//`；未选中时注释当前行。如果所有被操作的行已以 `//` 开头（忽略行首空白），则移除 `//` 取消注释。调试时频繁注释代码块，是最高频的编辑器操作之一。

### Indent / Unindent（Tab / Shift+Tab 有选区时）

选中多行文本后，Tab 键对每行增加一级缩进（Tab 模式加 `\t`，Space 模式加 indent_size 个空格）；Shift+Tab 对每行减少一级缩进。无选区时 Tab 插入制表符，Shift+Tab 无特殊行为。

### Duplicate Line（Ctrl+D）

光标所在行（或选中行）复制一份插入到下一行，不替换剪贴板内容。修改代码时经常需要复制一行再微调，比手动复制粘贴更高效。

### Delete Line（Ctrl+Shift+K）

删除光标所在行或选中的多行，比选中整行再 Delete 更快捷。

### Move Line Up / Move Line Down（Alt+Up / Alt+Down）

当前行（或选中行）与上方/下方行互换位置，用于调整代码行顺序。

### 编辑器右键菜单

CodeEditor 右键弹出上下文菜单，包含以下常用编辑动作：Undo、Redo、Cut、Copy、Paste、Comment/Uncomment、Indent、Unindent、Duplicate Line、Delete Line。菜单项与 Edit 菜单和快捷键共享同一动作，提供多入口操作方式。
...
- **编译器路径**：编译器的路径（默认使用 PATH 中的 gcc）。设置面板中提供"Auto Detect"按钮和"Browse..."按钮，可自动检测或手动选择系统中常见安装路径下的 g++/gcc 编译器（如 MinGW、TDM-GCC、Dev-C++ 自带 MinGW 等）
- **编译参数**：额外的编译选项（如 `-std=c++14`、`-O2` 等），默认留空，修改编译参数后会触发重新编译（编译产物比上次修改编译参数的时间旧则重编译）
...
- **编辑器字体与字号**：CodeEditor 的默认字体（独立于 IO 面板配置），必须为 monospace 字体。默认字体按平台自动选择：Windows 优先 Consolas，macOS 优先 Menlo，Linux 优先 DejaVu Sans Mono；系统未安装首选字体时依次尝试次选（Windows: Courier New，macOS: SF Mono，Linux: Ubuntu Mono），全部不可用时回退 Qt 的 monospace 系统默认。用户可在 Settings 中自定义覆盖
...
Qt 内部使用 Unicode，编辑器显示永远正确——关键是读文件时用检测到的编码解码。状态栏右侧显示当前文件编码（如 `Ln 4/120, Col 12 | UTF-8 | INS`），让用户知道文件编码状态。
...
保存时使用检测到的原始编码写回，不做编码转换。保存时自动去掉每行末尾多余空格/Tab（行尾空白清理）。用户如需改变编码，可通过 File > Reopen with Encoding 重新解释后再 Save with Encoding。
