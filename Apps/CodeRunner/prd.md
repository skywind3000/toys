...
工具栏下方的标签栏，每打开或新建一个源代码文件，会占用一个独立的 TAB 页，以文件名命名，新建的文件显示为 "untitled1"、"untitled2"（递增编号）。新建的文件内部带有"新文件"标志，表示该文件从未保存到磁盘；打开已存在的文件则没有此标志。文件名右边会显示是否有未保存的更改，基本行为参考标准的多文档文本编辑器。
