...
- 显示行号
- 当前行高亮：光标所在行用浅色背景标记，高亮条铺满编辑区整行宽度（不受行内容长度影响），在长代码中快速定位当前编辑位置
- Tab 键：无选区时插入 Tab 制表符（非空格），有选区时对选中行增加一级缩进
- Shift+Tab：有选区时对选中行减少一级缩进，无选区时反缩进当前行
- Ctrl+]：缩进当前行或选中行
- Ctrl+[：反缩进当前行或选中行
...
| Indent | 选中行增加一级缩进 | Edit > Indent | 否 | Ctrl+] / Tab（有选区时） |
| Unindent | 选中行减少一级缩进 | Edit > Unindent | 否 | Ctrl+[ / Shift+Tab（有选区时） |
...
选中多行时对每行行首添加 `// `（含空格）；未选中时注释当前行。如果所有被操作的行已以 `//` 开头（忽略行首空白），则移除 `//` 及紧随的空格取消注释。调试时频繁注释代码块，是最高频的编辑器操作之一。

### Indent / Unindent（Ctrl+] / Ctrl+[ / Tab / Shift+Tab）

选中多行文本后，Tab 键或 Ctrl+] 对每行增加一级缩进（Tab 模式加 `\t`，Space 模式加 indent_size 个空格）；Shift+Tab 或 Ctrl+[ 对每行减少一级缩进。无选区时 Tab 插入制表符，Ctrl+] 缩进当前行，Shift+Tab 或 Ctrl+[ 反缩进当前行。
