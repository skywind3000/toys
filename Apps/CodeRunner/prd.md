...
- Test 运行后程序的标准输出（stdout）和标准错误（stderr 合并到 stdout，不做区分），以及返回码和运行耗时；返回码非 0 时显示为 Runtime Error
- 编译错误信息（以**红色字体**显示），正常程序输出使用默认字体颜色
- Build 的编译结果

**OutputPanel 的内容是 per-tab 的，切换标签时输出内容随之切换。**
...
| Stop | 终止当前运行的进程 | Run > Stop | 是 | Escape |
...
点击 Test 时：先保存文件（如果有更改；如果当前文件是 UNNAMED 则弹出保存对话框，用户取消保存则终止后续流程），然后如果没有最新的编译产物就重新编译（"最新"指：可执行文件不存在，或者可执行文件存在但比源文件旧），如果编译出错就在 OutputPanel 显示错误信息，然后结束；如果编译正常，就从 InputPanel 里面取得输入内容，作为启动程序的标准输入，然后捕获标准输出（stderr 合并到 stdout，不做区分）、返回码和运行耗时显示到 OutputPanel。返回码非 0 时显示为 Runtime Error。如果运行超时，则终止进程并在 OutputPanel 末尾以**红色字体**显示 "Timeout after xx seconds"。正常运行结束后，在输出内容下方显示退出码、运行耗时和内存占用（如 "exit with code 0 in 0.015s, 1.2MB"）。

### Run（交互式运行）

点击 Run 时：先保存文件（如果有更改；如果当前文件是 UNNAMED 则弹出保存对话框，用户取消保存则终止后续流程），然后如果没有最新的编译产物就重新编译（同 Test 的判断逻辑），如果编译出错就在 OutputPanel 显示错误信息，然后结束；如果编译正常，就弹出一个系统终端窗口运行程序，程序结束后提示"按任意键关闭"，用户可在终端窗口中手动输入数据进行交互。
...
如果一次 Test 或 Run 过程没结束，则不能开启下一次，除非按 Stop（Escape）终止当前进程。运行过程有时间限制，默认 10 秒，可以在设置里更改。超时后终止进程，并在 OutputPanel 末尾以红色字体显示 "Timeout after xx seconds"。
