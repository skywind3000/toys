...
图标生成函数：`_generate_new_icon()` / `_generate_save_icon()` / `_generate_open_icon()` / `_generate_run_icon()` / `_generate_test_icon()` / `_generate_stop_icon()` / `_generate_settings_icon()`，由 `_create_toolbar_icons()` 统一创建返回 dict。所有 QAction 通过 `QAction(icon, text, self)` 构造，toolbar 显示纯图标，菜单显示文字。
