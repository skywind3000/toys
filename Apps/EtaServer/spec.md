...
- 模板内 `include("./header")` 仍由 Eta 解析（Eta v3 模板函数名为 `include`，非 v2 的 `includeFile`；renderStringAsync 编译时未传 filepath，故相对 `views`（HTTP 模式即文档根、CLI 模式即脚本所在目录/cwd，见决策 #11）解析），与 Mako 的 include 相对当前模板目录的语义有差异，文档注明即可。
...
- 密钥派生：`SHA256('eta-server|hostname|username|homedir|首个非零MAC|realpath(root)')`，不落盘、同机稳定、跨机不同、**同机不同 root 亦不同**（决策 #13）；`os.userInfo()` / `networkInterfaces()` 均带 try 兜底；
...
- 静态白名单见 `STATIC_TYPES`（38 种，与 MakoServer 对齐：网页 / 文本 / 数据 / 图片 / 字体 / 音视频 / wasm / 压缩包，`.js`/`.mjs` 为 `text/javascript`），白名单外 404；存在但谓词非 GET/HEAD → 405 + `Allow: GET, HEAD`（白名单优先，404 不因 405 泄露存在性）；
...
### 决策 #11：CLI 渲染模式（对标 MakoServer 命令行渲染）
...
- **模式判定**：parseArgs 遇首个非选项位置参数即进 CLI 模式，其后一律 break（脚本名后的参数即使形如 `-p` 也原样透传，同 MakoServer 的 argparse REMAINDER）；无位置参数才进 HTTP 服务模式。位置参数出现在 `-r` / `-p` / `-H` 之前或之后均可（选项照常吃掉各自的值）；
...
- **不实现 PHP exit() flush 语义**（MakoServer 决策 #36 的对应物）：Eta 渲染是纯函数，输出仅在渲染完整返回后一次性产出，无法在中途 flush 已缓冲内容；脚本中途停止输出用顶层 `return`（模板代码块在函数体内），强行 `process.exit(code)` 会丢弃全部已生成输出直接退出（Node 语义，不设防、不兜底）；
- **不读任何配置**（本无配置文件机制）；`-r` / `-p` / `-H` 在 CLI 模式静默接受但不产生作用（PHP 式宽容，对标 MakoServer 10.3）；
- `renderCli(script, args)` 一并导出（module.exports），供测试 / 编程调用。

### 决策 #12：路径加固（realpath 容器校验 + Windows 特判）

字符串前缀检查只挡得住文本层穿透，挡不住文件系统层的逃逸，补齐对标 MakoServer 的一整套：
...
对标 MakoServer 的三项 session 健壮性机制：

- **4KB 上限**（`SESSION_COOKIE_LIMIT = 4096`）：重签时校验整条 Set-Cookie 字节长（`Buffer.byteLength`），超限 → 500（错误页含 "about 4KB" 字样）。浏览器对超大 cookie 静默丢弃，不如显式失败（MakoServer SessionTooLarge 同语义）；
- **密钥混入 root**：`deriveSecret(rootDir)` 的 seed 追加 `realpathSync(root)`——同机不同文档根的站点密钥不同，A 站 session cookie 投 B 站验签失败按无 session 处理（MakoServer 决策 #34 对应物）；CLI 模式不用 session 不参与；函数一并导出供测试；
- **session cookie 名独占**：`RESP.setcookie('etasess', ...)` 与框架 session 同名，该条丢弃不下发并向 stderr 打 warning（下发两条同名 Set-Cookie 的浏览器取舍未定义）；`RESP.header('Set-Cookie', ...)` 原始通道不做名称检查（同 MakoServer 逃生舱哲学）。
...
| `RESP` | object | `status/header/redirect/setcookie/json/writeraw/escape`（status 非 100–999 整数 → 500；setcookie 与 session 同名 → 丢弃，决策 #13） |
...
- API：GET echo、form POST、JSON body、**+json Content-Type 进 `_JSON`**；
- bridge：**HTTP `_SERVER` 含 SERVER_PROTOCOL / REQUEST_TIME(_FLOAT)**；
- session：三级链式 cookie 计数 1→2→3、篡改签名拒绝（count 归 1）、**超 4KB → 500、deriveSecret 混 root 单元断言、setcookie 同名独占（不下发 + 仅框架一条）**；
- RESP：**status(9999) → 500、RESP.escape 与 escape() 等价**；
- require：demo 页渲染；
- 500：broken.eta（引用未定义函数）。

跑法：`npm test`（Node 18+，用全局 fetch；依次跑 test_server.js 与 test_cli.js）。
...
- 无配置文件/日志；session TTL 硬编码 30 分钟、仅 sliding 模式、cookie 名固定（MakoServer 的 session_mode / session_cookie / session_lifetime 配置属配置文件二期）；
- `_GET` / `_POST` / `_REQUEST` 为普通对象，同名参数多值后覆盖前（无 MakoServer PHPDict 的 `getlist` 多值访问）；
