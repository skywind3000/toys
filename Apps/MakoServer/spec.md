...
| 25 | 注入名作用域属模板编写约定而非框架缺陷 | demo 留言板实测踩坑：`<%! %>`（模块级）里定义的辅助函数调用 `escape` → 编译正常、**运行时**首次被调才报 `NameError`（注入名仅存在于渲染作用域，见 6.7）。框架不做任何修复/兜底——Mako 两级作用域系模板引擎固有语义，强行把注入名塞进模板模块 globals 会引入跨请求共享可变状态（与 9.4 每请求隔离原则冲突）；以文档约定收口：`<%! %>` 只放 import 与纯函数，要碰注入名的辅助函数放 `<% %>` 或显式传参（PRD「注入名作用域」节，spec 6.7） |
...
| 37 | application 惰性构造（import 零副作用；mod_wsgi 实机修订） | 旧实现 import 即跑配置查找链（含 `~/.config` 用户级配置污染）+ `validate_startup` 失败 `sys.exit(1)` 杀宿主进程 + `derive_host_secret()`（Windows 起 wmic 子进程）。**初版修正用 PEP 562 模块级 `__getattr__` 惰性——实机部署即翻车**：mod_wsgi 解析目标 callable 是对脚本命名空间**字典直查**（`Target WSGI script does not contain WSGI application 'application'`），不走模块属性协议，`__getattr__` 永不触发（gunicorn / `from ... import` 走属性访问，测试全绿恰好漏掉首要部署目标）。**终版**：`application = LazyApplication()`——真实模块级绑定（mod_wsgi 字典直查可命中），WSGI 可调用对象，**首个请求**双检锁构建真实 app 并缓存（threaded 宿主并发首请求只建一个）；`import makoserver` 仍零副作用；`_wsgi_bootstrap` 失败抛 `ConfigError`（stderr 留一行），经首请求交宿主呈现，不再 `sys.exit` |
...
| 输出 | `make_echo`（文本 echo，缓冲为 `io.StringIO`）/ `html_escape` |
| Bridge | `PHPDict` / `RespObject`（内含 writeraw 独立二进制缓冲）/ `make_bridge()` |
...
- 其余键（`secret` / `session_*` / `max_body` / `access_log` / `error_log`）：仅配置文件 > 内置默认（无命令行对应参数）；
...
7. **多站点同进程风险声明**：`sys.path` 是进程级集合——同进程挂多个 root（mod_wsgi 同 daemon 多应用 / gunicorn 单进程多 app）时各 root 依次追加，py3 命名空间包按 `sys.path` 顺序先到先得，**两个站点的同名顶层包（如都叫 `common/`）会互相串站**（先追加的 root 胜出，后者静默拿到别人的模块）。规避方式二选一：各站点顶层包名自行错开（如 `site1_common`），或生产部署给每个站点独立进程（mod_wsgi 独立 daemon process group / gunicorn 分实例——prefork 隔离本就是 9.4 推荐形态）。框架不做 import 隔离（PEP 302 钩子级隔离的复杂度远超本项目定位）。
...
自定义模板集合（历史上因 rstrip 钩子弃用 `mako.lookup.TemplateLookup`；决策 #30 删除 rstrip 后该理由消失，但 mtime_ns+size 缓存、utf-8-sig BOM 读取与钉死的 not-found 错误形态均已有测试覆盖，保留不重构），实现 `get_template(uri)` 与 `adjust_uri(uri, relativeto)` 两协议，供 `Template(text=..., lookup=store)` 的 include/inherit 解析。

### 4.1 加载与编译

1. `path = os.path.join(base_dir, *uri.split('/'))`；
2. 读源码：`open(path, 'r', encoding='utf-8-sig')`（兼容 Windows 记事本 BOM）；
3. **源码按原文编译，不做尾部空白截断**（决策 #30：文本模板忠实保留源文件尾部换行——主模板、include、inherit、CLI 脚本、stdin 一致；二进制脚本的 EOF 换行由 writeraw 短路机制天然丢弃，无需 rstrip）；
...
## 5. 输出缓冲（文本 / 二进制双通道，决策 #30）

### 5.1 文本缓冲

普通 `io.StringIO`：模板文本块与 `echo()`（str / str() 后的其它类型）共享同一实例、顺序交错写入；组装时 `getvalue().encode('utf-8')` 得到文本 body。不再有自定义 buffer 类。

### 5.2 与 Mako 的集成（只用公开 API）

渲染走 Mako 官方 Usage 文档 "using the Context programmatically" 模式：
...
挂在 `RespObject` 内部：chunk 列表 + `raw_used` 标志。`writeraw(*args)` 逐参数校验 bytes-like（bytes / bytearray / memoryview → `bytes()` 追加；其它类型抛 TypeError），**进入函数即置 `raw_used = True`（零参数调用也算）**；组装阶段 `collect_raw()` 返回 `(raw_used, b''.join(chunks))`，raw_used 为真时 body 取二进制缓冲、文本缓冲整体丢弃（短路语义，渲染期 echo 照常写文本缓冲不报错）。

### 5.4 异常丢弃

渲染抛异常时文本 / 二进制两个缓冲一并丢弃，直接进入 500 流程。

## 6. Bridge

### 6.1 注入机制

`template.render(context, **bridge_names)` 或经 5.2 的 Context 构造注入。注入名固定为下表 12 个，未注入名在模板中引用时按 Mako 默认 NameError 报 500。
...
|--------|------|------|
| `echo` | function | 见 6.2 |
| `echoraw` | bound method | = `RESP.writeraw`（同一绑定方法，见 6.2 与 5.3） |
...
| `_COOKIE` | dict | `request.cookies`，值逐个 `urllib.parse.unquote`（PHP `$_COOKIE` urldecode 的对应物，与 `RESP.setcookie` 写侧编码配对；**字面 `+` 保留不解为空格**——与 PHP 的有意分歧，见决策 #31；session codec 直读 `request.cookies` 不经此解码） |
| `_SESSION` | dict | 见 7 |
...
**echo（文本通道）**：逐参数顺序写入文本缓冲（StringIO）：`None` → 跳过（空串，对齐 PHP `echo null`）；`str` → 直接写；**`bytes/bytearray/memoryview` → 抛 `TypeError`**（消息指引 `RESP.writeraw()/echoraw()`，HTTP 下 500、CLI 下 exit 1——二进制不允许流入文本通道，决策 #30）；其它 → 写 `str(x)`。多参数按序全部输出后返回（无返回值）。

**echoraw / RESP.writeraw（二进制通道）**：逐参数追加到独立二进制缓冲：`bytes/bytearray/memoryview` → `bytes(x)` 追加；**其它类型（含 str / None）→ 抛 `TypeError`**（消息指引 echo，通道严格对偶）。调用过即置 raw 短路标志（零参数也算），组装语义见 5.3 / 8；不设置 Content-Type（脚本自行 `RESP.header`）。CLI 模式照常生效（输出而非响应控制，不受 no-op 规则影响）。
...
`_BODY` 为原始 body bytes（CLI 恒 `b''`）。大小受配置键 `max_body` 约束（默认 64MB，映射 Flask `MAX_CONTENT_LENGTH`，超限 Werkzeug 标准 413，`<= 0` 不限制，决策 #32），限内 body 全量读入内存（可信环境定位，公网加固另见 hardening.md）。`_JSON`：Content-Type 字符串包含子串 `json`（覆盖 `application/json`、`+json`、`text/json`）时 `json.loads(_BODY)`，**解析失败 → None**；不含 json → None。body 为空也 → None。
...
12 个注入名经 `template.render(context, **bridge)` 传入，是 `render_body` 的 kwargs/局部名，**不在模板模块的 globals 里**。因此 `<%! %>` 中定义的函数调用时按常规 Python 规则在模块 globals 查名，查不到 `escape` / `echo` / `_GET` 等 → `NameError`。失败形态的三个延迟叠加使其极具迷惑性：① 编译不报错（注入名是合法标识符）；② 运行时才查名（函数被调用那一刻）；③ 若该函数仅特定分支调用（如 `?debug=1` 面板），平时页面一切正常。框架不做修复/兜底（决策 #25）。
...
1. body：`resp.collect_raw()` 的 raw 标志为真 → **writeraw 短路**，body = 二进制缓冲字节、文本缓冲整体丢弃；否则 body = 文本缓冲 `getvalue().encode('utf-8')`（决策 #30）；
2. status = RESP 记录的 code（未设 → 200）；
3. Content-Type：RESP 显式设置值 > 默认 `text/html; charset=utf-8`（writeraw 不代设，短路场景未显式设置时同样落默认值）；
...
`application` 为模块级 **LazyApplication 惰性包装对象**（决策 #37 修订版）：真实的模块级绑定——mod_wsgi 对 WSGI 脚本命名空间做**字典直查**取目标 callable，不走模块属性协议（PEP 562 `__getattr__` 方案实机翻车的教训），普通绑定对 mod_wsgi / gunicorn / uWSGI / `from makoserver import application` 全部可见；`import makoserver` 零副作用（不查配置链、不校验启动、不派生密钥——pytest 收集 / 工具 import 不再被用户级配置污染或被 `sys.exit` 杀死）；**首个请求**到达时双检锁构建真实 app（每进程一次，无跨进程状态，天然多进程兼容；threaded 宿主并发首请求只建一个）。构造失败抛 `ConfigError`（stderr 同步留一行），经首请求的 500 交宿主呈现，不再 `sys.exit(1)`——注意配置错误由此**延迟到首请求**才暴露，Apache 启动本身不报错。root 见 2.4（配置缺失时回退本文件所在目录，零配置即拷即用）。Apache `WSGIScriptAlias /app1 /path/to/makoserver.py` 即用。
...
| 文件 | 覆盖点 |
|------|--------|
| `test_config.py` | 四级查找命中即用（`--conf` > `MAKOSERVER_CONF` > 同目录 ini > settings.ini）；`MAKOSERVER_CONF` 文件不存在继续下寻；相对路径基准（root/log 路径均相对配置文件目录）；`-r` 覆盖配置 `root`；配置含 `port` 键被忽略（不报错）；非法 INI 报错（缺 `[makoserver]` 节 / 解析错 / `session_lifetime` 非整数）；**含 `%` 的 secret 正常读出（interpolation=None 回归，不抛 InterpolationSyntaxError）**；注释行与未知键忽略；dev server root 三级回退（-r > 配置 > cwd）；WSGI 零配置回退 makoserver.py 所在目录；**root 不存在 / 非目录 → 启动报错退出（stderr 含 root 路径与来源；决策 #22）**；**`error_log` / `access_log` 父目录不存在 / 不可写 → 启动报错退出（append 探测失败；决策 #22 同哲学）**；**`max_body`：非整数 → 报错退出、自定义小值生效、`<= 0` 不限制（决策 #32）**；**`session_lifetime <= 0` → 报错（决策 #38）**；**`static_types` 非法格式 → 报错（决策 #39）**；**`--conf` 指向不存在文件 → 报错（决策 #40，原「继续下寻」行为反转）**；**import 纯净性（决策 #37 修订版）：子进程断言 import 不构建真实 app（`application.app is None`）、`application` 为模块级真实绑定（mod_wsgi 字典直查回归）、首个 WSGI 请求（werkzeug Client 直调包装对象）触发构建** |
