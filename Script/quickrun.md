...
`configure` 类在初始化时会构建环境变量表，所有值均为字符串。命令字符串中必须使用 `$(NAME)` 形式引用变量，例如 `$(FILENAME)`；单独的 `FILENAME` 或 `$FILENAME` 不会被替换。
...
变量替换发生在执行前：工具会扫描命令字符串中的 `$(NAME)` 占位符，并替换为环境变量表中对应值。仅支持 `$(NAME)` 这一种写法。
